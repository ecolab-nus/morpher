...
def main():
  ...
  MAPPER_KERNEL = MAPPER_HOME + '/applications/hycube8x8/array_add_agu/'
  SIMULATOR_KERNEL =SIMULATOR_HOME + '/applications/hycube8x8/array_add_agu/'
...
##############################################################################################################################################
  ...
  II_right = 0
  II_left = 0
  print('../../../build/src/cgra_xml_mapper -d array_add_agu.xml -x 4 -y 4 -j hycube_original_mem.json -t HyCUBE_4REG')

  os.system('../../../build/src/cgra_xml_mapper -d array_add_agu.xml -x 4 -y 4 -j hycube_original_mem.json -t HyCUBE_4REG')
  for file in os.listdir("./"):
    if file.endswith(".bin"):
        II_left=int(file.split('II=')[-1].split(' ')[0].split('_MTP')[0].split(' ')[0])+1
    os.system('mv *.bin binary/left.bin')
  os.system('../../../build/src/cgra_xml_mapper -d array_add_agu.xml -x 4 -y 4 -j hycube_original_mem_RC.json -t HyCUBE_4REG')
  for file in os.listdir("./"):
    if file.endswith(".bin"):
