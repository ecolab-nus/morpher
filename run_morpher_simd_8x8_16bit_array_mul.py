...
def main():
  ...
  DFG_GEN_KERNEL = DFG_GEN_HOME + '/benchmarks/pace_chip_experiments/array_mul/' 
  MAPPER_KERNEL = MAPPER_HOME + '/applications/hycube8x8/array_mul_simd/'
  SIMULATOR_KERNEL =SIMULATOR_HOME + '/applications/hycube8x8/array_mul_simd/'
...
#   print('\nGenerating DFG\n')
#   os.system('./run_pass.sh array_mul 2 8192')
#   os.system('dot -Tpdf array_mul_PartPredDFG.dot -o array_mul_PartPredDFG.pdf')
...
#   os.system('cp array_mul_mem_alloc.txt '+SIMULATOR_KERNEL)
#   os.system('cp array_mul_mem_alloc.txt '+MAPPER_KERNEL)
#   os.system('cp array_mul_PartPredDFG.xml '+ MAPPER_KERNEL)
...
##############################################################################################################################################
  ...
  os.system('python ../../../update_mem_alloc.py ../../../json_arch/hycube_simd_updatemem.json array_mul_mem_alloc.txt 8192 2 hycube_simd_mem.json')
  os.system('python ../../../update_mem_alloc.py ../../../json_arch/hycube_simd_updatemem_RC.json array_mul_mem_alloc.txt 8192 2 hycube_simd_mem_RC.json')
  print('\nupdate memory allocation done!\n')

  II_right = 0
  II_left = 0
  os.system('../../../build/src/cgra_xml_mapper -d array_mul_simd.xml -x 4 -y 4 -j hycube_simd_mem.json -t HyCUBE_4REG')
  for file in os.listdir("./"):
    if file.endswith(".bin"):
        II_left=int(file.split('II=')[-1].split(' ')[0].split('_MTP')[0].split(' ')[0])+1
    os.system('mv *.bin binary/left.bin')
  os.system('../../../build/src/cgra_xml_mapper -d array_mul_simd.xml -x 4 -y 4 -j hycube_simd_mem_RC.json -t HyCUBE_4REG')
  for file in os.listdir("./"):
    if file.endswith(".bin"):
...
##############################################################################################################################################
  ...
  os.system('mv array_mul_mem_alloc.txt mem_alloc.txt')
  os.system('python3 ../../../scripts/duplicate_config.py > dup.log')

  # for invocations in range(0,num_memory_traces) :
  #   data_file = SIMULATOR_KERNEL + '/memtraces/array_mul_trace_' + str(invocations) + '.txt'
  ...
  if invocation > 0 :
   for i in range(0,invocation) :
    print('../../../src/build/hycube_simulator duplicated_config.bin data_modi_' + str(i*4) +'.txt mem_alloc.txt 8 8 16384 data_modi_' + str((i*4 + 1)) + '.txt data_modi_' + str((i*4 + 2)) + '.txt data_modi_' + str((i*4 + 3)) +'.txt | tail -n 2 |head -n 1 > output.log')
    ...
    if out_file == 'dumped_raw_data_i0':
      BT.dump_trace_full("duplicated_config.bin",'dataDump/'+out_file + '.txt',II_left,4,"array_mul")
      os.system('mv *.trc *.h traces/')
    else:
      os.system('mkdir traces/'+str(i))
      BT.dump_trace_no_cmem('dataDump/'+out_file + '.txt',II_left,4,"array_mul")
      os.system('mv *.trc *.h traces/'+str(i))


  if half_invocations == 1 :
    print('../../../src/build/hycube_simulator duplicated_config.bin data_modi_' + str((invocation*4)) +'.txt mem_alloc.txt 8 8 16384 | tail -n 2 |head -n 1 > output.log')
    ...
    os.system('cp dumped_raw_data.txt dataDump/'+ out_file + '.txt')
    if out_file == 'dumped_raw_data_i0':
      BT.dump_trace_full("duplicated_config.bin",'dataDump/'+out_file + '.txt',II_left,half_invocations,"array_mul")
      os.system('mv *.trc *.h traces/')
    else:
      os.system('mkdir traces/'+str(invocation))
      BT.dump_trace_no_cmem('dataDump/'+out_file + '.txt',II_left,half_invocations,"array_mul")
...
#############################################################################################################################################
  ...
  for filename in os.listdir('dataDump'):
      if filename == 'dumped_raw_data_i0.txt':
        BT.dump_trace_full("duplicated_config.bin",'dataDump/'+filename,II_left,half_invocations,"array_mul")
      else:
