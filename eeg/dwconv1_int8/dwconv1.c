...
int8_t data[5640];
int8_t kernel[60];
int8_t dwconv1[OUTLEN];// tvm target: c -keys=cpu 
...
TVM_DLL int32_t conv_main(void* args, int32_t* arg_type_ids, int32_t num_args, void* out_ret_value, int32_t* out_ret_tcode, void* resource_handle) {
  ...
  for (int32_t ow_outer = 0; ow_outer < 1; ++ow_outer) {
    ...
    int32_t mod_12 = 0;
    for (int32_t ow_inner_c_fused_m_fused = 0; ow_inner_c_fused_m_fused < OUTLEN; ++ow_inner_c_fused_m_fused) {
      #ifdef CGRA_COMPILER  
      please_map_me();
      #endif
...
int main() {
    ...
    // write out tensor
    if (write_int8_data("dwconv1_output.txt", dwconv1, OUTLEN) != 0) {
